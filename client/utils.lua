...
local GetVehicleInteriorColor = GetVehicleInteriorColor
local GetVehicleDashboardColour = GetVehicleDashboardColour
local GetVehicleWheelWidth = GetVehicleWheelWidth
local GetVehicleWheelSize = GetVehicleWheelSize
...
function Utils.getNearbyVehicles(coords, maxDistance)
...
RegisterNetEvent('pip-speedtraps:setVehicleProperties', function(netid, data)
    local timeout = 100
    ...
    if timeout > 0 then
        Utils.setVehicleProperties(NetToVeh(netid), data)
    end
end)

---@param vehicle number
---@return VehicleProperties?
function Utils.getVehicleProperties(vehicle)
...
---@return boolean?
function Utils.setVehicleProperties(vehicle, props)
    if not DoesEntityExist(vehicle) then error(("Unable to set vehicle properties for '%s' (entity does not exist)"):
            format(vehicle))
    end
    if NetworkGetEntityIsNetworked(vehicle) and NetworkGetEntityOwner(vehicle) ~= PlayerPedId() then error((
